...
|  #  | 이벤트    | 인자                    | 클라이언트 API (`Rooms.ts`)           | 서버 API (`app.ts`)  | 서버 동작                                            |
| :-: | :-------- | :---------------------- | :------------------------------------ | :------------------- | :--------------------------------------------------- |
|  1  | 방 입장   | `id`, `room`            | `socket.emit('join-room', id, room)`  | `socket.join(room)`  | `join` 후 알림·인원 갱신 broadcast · → S→C **1, 2**  |
|  2  | 방 퇴장   | `id`, `room`            | `socket.emit('leave-room', id, room)` | `socket.leave(room)` | 알림·인원 갱신 broadcast 후 `leave` · → S→C **1, 3** |
|  3  | 채팅 전송 | `{ name, msg }`, `room` | `socket.emit('chatting', data, room)` | —                    | relay만 함 · → S→C **4**                             |

### Server → Client

| API               | 수신 대상                                 |
| :---------------- | :---------------------------------------- |
| `socket.to(room)` | 같은 room, **본인 제외** (보낸 소켓 빠짐) |
| `io.in(room)`     | 같은 room, **본인 포함** (room 전체)      |
...
| 구분   | 항목           | 내용                                      |
| :----- | :------------- | :---------------------------------------- |
| HTTP   | 인증           | 세션 없음 — `userId` 위조 가능            |
| HTTP   | 보안           | 비밀번호 DB 평문 저장                     |
| HTTP   | `/api/users`   | SQL 3회 비동기 race                       |
| HTTP   | `/api/friends` | JOIN `date`가 본인 가입일일 수 있음       |
| HTTP   | 중복           | 친구 요청·이미 친구 검사 없음             |
| Socket | roomData       | 재시작 시 참여자 초기화                   |
| Socket | disconnecting  | emit 형식 버그 (`chatting` 이벤트명 누락) |
| 프론트 | DM             | 미구현 (`alert`만)                        |
| 프론트 | 로그인 UX      | 성공 후 `.main`만 비움                    |
| 설정   | DB             | `app.ts` 하드코딩 → `.env` 권장           |
